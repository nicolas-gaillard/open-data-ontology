...
function buildCategorySelect() {
  ...
  const data = sendQuery(query, function(err, data) {
    ...
    for (row in bindings) {
      ...
      select.append(
        $("<option/>")
          .attr("value", "n1:" + category)
          .text(category.split("category_")[1].replace(/_/g, " "))
      );
    }
  });
}
...
function buildBrewerSelect() {
  ...
  const data = sendQuery(query, function(err, data) {
    ...
    for (row in bindings) {
      select.append(
        $("<option/>")
          .attr("value", bindings[row].name.value)
          .text(bindings[row].name.value)
      );
    }
  });
}
...
function buildCountrySelect() {
  ...
  const data = sendQuery(query, function(err, data) {
    ...
    for (row in bindings) {
      select.append(
        $("<option/>")
          .attr("value", bindings[row].pays.value)
          .text(bindings[row].pays.value)
      );
